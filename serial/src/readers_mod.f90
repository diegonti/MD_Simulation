...
subroutine read_nml(param_file, lj_epsilon, lj_sigma, mass, timestep, density, &
    ...
    n_sweeps, write_frame, sim_name, cell_type, init_velocities, temperature)
    implicit none
    ...
    ! In/Out variables
    real(kind=dp), intent(inout)         :: lj_epsilon, lj_sigma, mass, timestep, density, andersen_nu, temperature
    integer(kind=i64), intent(inout)     :: n_particles, n_steps, write_file, write_stats, gdr_num_bins, &
                                            n_sweeps, write_frame
    ...
    andersen_nu, gdr_num_bins, n_sweeps, cell_type, &
    init_velocities, temperature
